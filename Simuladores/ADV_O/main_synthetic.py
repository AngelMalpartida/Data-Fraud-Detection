...
def make_classification(train_size_days=10, test_size_days=10, load = True):
    ...
    # Guardar el DataFrame transactions_df como CSV
    output_dir = './Simuladores/Output/ADV-O/'
    ...
    # Verificar si transactions_df contiene datos
    if transactions_df.empty:
        print("Error: transactions_df está vacío. No se guardará el archivo.")
    else:
        transactions_df.to_csv(os.path.join(output_dir, 'transactions_df.csv'), index=False)
        print(f"Archivo guardado en: {os.path.join(output_dir, 'transactions_df.csv')}")

if __name__ == '__main__':
    np.random.seed(RANDOM_STATE)
    make_classification(train_size_days=20, test_size_days=20, load=False)
